...
export class O365RestoreSession extends Entity {
   ...
   * This method will return TaskResponse object immediately and submits the restoration task in the background.
   * @param {string} oneDriveId
  ...
  async restoreOneDriveFolder(oneDriveId: string, folderId: string,
                              request: O365OneDriveRestoreOptionsRequest): Promise<Task> {
    return Iland.getHttp()
        .post(
            `/o365-restore-sessions/${this.uuid}/onedrive-drives/${oneDriveId}/folders/${folderId}/actions/restore`,
            request.json
        )
        .then((response) => {
          const json = response.data as TaskJson;
