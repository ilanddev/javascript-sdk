...
export class O365RestoreSession extends Entity {
   ...
   * @param {O365RestoreCredentialsRequest} request
   * @returns {Promise<Task>}
   */
  /* istanbul ignore next: autogenerated */
  async restoreMailbox(mailboxId: string, request: O365RestoreCredentialsRequest): Promise<Task> {
   ...
   */
  /* istanbul ignore next: autogenerated */
  async getO365DeviceCode(applicationId: string): Promise<O365DeviceCode> {
    // tslint:disable-next-line:max-line-length
    return Iland.getHttp().get(`/o365-restore-sessions/${this.uuid}/actions/get-o365-device-code?applicationId=${applicationId}`)
        .then((response) => {
