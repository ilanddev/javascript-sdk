...
import {
  VdcIntegratedBackupStatusDetail
} from '../integrated-backups/integrated-backup-status/vdc-integrated-backup-status-detail';
import {
...
export class Vdc extends Entity implements EntityWithPerfSamples {
   ...
   */
  /* istanbul ignore next: autogenerated */
  async getIntegratedBackupStatus(): Promise<VdcIntegratedBackupStatusDetail> {
    return Iland.getHttp().get(`/vdcs/${this.uuid}/integrated-backup-status`).then((response) => {
      const json = response.data as VdcIntegratedBackupStatusDetailJson;
