...
import { PerformanceCounter } from '../common';
import { PerformanceCounterJson } from '../common/__json__/performance-counter-json';
...
export class Vpg extends Entity {
  ...
  downloadFailoverReport(reportUuid: string, fileName?: string): Observable<String> {
     ...
     * @return {Promise<Array<PerformanceCounter>>} performance counters
     */
    /* istanbul ignore next: autogenerated */
  async getPerformanceCounters(): Promise<Array<PerformanceCounter>> {
    return Iland.getHttp().get(`/vpgs/${this.uuid}/performance-counters`).then((response) => {
      const json = response.data.data as Array<PerformanceCounterJson>;
      return json.map((it) => new PerformanceCounter(it));
    });
  }
}
