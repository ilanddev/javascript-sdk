...
import { VccFailoverStartCreateRequest } from './failover-plan-start-request';
...
export class VCCFailoverPlan extends Entity {
   ...
   */
  /* istanbul ignore next: autogenerated */
  async failoverStart(spec: VccFailoverStartCreateRequest): Promise<Task> {
    return Iland.getHttp().post(`/vcc-failover-plans/${this.uuid}/actions/failover-start`, spec.json)
        .then((response) => {
          const json = response.data as TaskJson;
          return new Task(json);
        });
