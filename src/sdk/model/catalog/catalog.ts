...
import { InitiateOVFUploadRequest } from './initiate-ovf-upload-request';
...
export class Catalog extends Entity {
   ...
   */
  /* istanbul ignore next: autogenerated */
  async initiateOVFUpload(body: InitiateOVFUploadRequest): Promise<Task> {
    return Iland.getHttp().post(`/catalogs/${this.uuid}/actions/initiate-ovf-upload`, body.json).then((response) => {
      const json = response.data as TaskJson;
      return new Task(json);
    });
  }
   ...
   */
  /* istanbul ignore next: autogenerated */
  async deleteCatalog(deleteTemplatesAndMedia: boolean = false): Promise<Task> {
