...
import { OrgIntegratedBackupStatus } from '../integrated-backups/integrated-backup-status/org-integrated-backup-status';
import {
OrgIntegratedBackupStatusJson
} from '../integrated-backups/integrated-backup-status/__json__/org-integrated-backup-status-json';
...
export class Org extends Entity {
  ...
  static async getVpgPerformanceCounters(): Promise<Array<PerfCounter>> {
    return Iland.getHttp().get('/orgs/performance-counters-vpg').then((response) => {
   ...
   */
  /* istanbul ignore next: autogenerated */
  async getIntegratedBackupStatus(): Promise<OrgIntegratedBackupStatus> {
    return Iland.getHttp().get(`/orgs/${this.uuid}/integrated-backup-status`).then((response) => {
      const json = response.data as OrgIntegratedBackupStatusJson;
      return new OrgIntegratedBackupStatus(json);
    });
  }
   ...
   */
  /* istanbul ignore next: autogenerated */
  async getOrgPerfFor(group: string, name: string, type: string,
                             start?: number, end?: number): Promise<PerfSamplesSeries> {
    return Iland.getHttp().get(`/orgs/${this.uuid}/performance/${group}::${name}::${type}`, {
      params: {
        start: start ?? null,
        end: end ?? null
      }
